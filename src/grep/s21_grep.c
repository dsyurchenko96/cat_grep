...
int main(int argc, char *argv[]) {
    ...
    if (exit_status == EXIT_SUCCESS && optarg == NULL) {
        exit_status =
            add_pattern(&patterns, argv[optind], strlen(argv[optind]));
...
int process_file(FILE *file, const char *filename, Flags flags,
    ...
    while ((line.len = getline(&line.string, &line.alloc_size, file)) != -1) {
        ...
        for (int pat_ind = 0; pat_ind < patterns->num_patterns; pat_ind++) {
            if (flags.o) {
                num_matches +=
                    process_o(&line, filename, patterns, pat_ind, flags);
            } else if (regexec(&patterns->regex_array[pat_ind], line.string, 0,
                               NULL, 0) == flags.v &&
                       !line.skip) {
...
int add_pattern(Patterns *patterns, const char *pattern_string, size_t len) {
    ...
    if (patterns->num_patterns == 0) {
        ...
        patterns->capacity *= 2;
        patterns->string_array = realloc(patterns->string_array,
                                         sizeof(char *) * patterns->capacity);
    }
    if (patterns->string_array == NULL) {
        ...
        strncpy(patterns->string_array[patterns->num_patterns - 1],
                pattern_string, len);
